...
def main(argv=None):
    ...
    with tf.Session(config=tf.ConfigProto(allow_soft_placement=True)) as sess:
        ...
        start = time.time()
        print('Initialize global step: {:.0f}, learning rate: {:.20f}'.format(sess.run(global_step),
                                                                              sess.run(learning_rate)))
        log = open('/gpu_data/logs/east.log', mode='w', encoding='utf8')
        for step in range(FLAGS.max_steps):
            ...
            if step % 10 == 0:
                ...
                start = time.time()
                msg = 'Step {:06d}, model loss {:.4f}, total loss {:.4f}, {:.2f} seconds/step, {:.2f} examples/second'.format(
